...
class Web100NowUpdate extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'web100now:update';
     ...
     * @var string
     */
    protected $description = 'Update Web100Now';
    ...
    public function handle()
    {
        ...
        foreach ($servers as $server) {
            ...
            $ssh->login('web100now', $server->password);
            $ssh->setTimeout(360);
        ...
        $ssh->login('web100now', $server->password);
        $ssh->setTimeout(360);
        $ssh->exec('echo '.$server->password.' | sudo -s cd /var/www/html/utility/web100now-update && sh run.sh');
        $ssh->exec('exit');
