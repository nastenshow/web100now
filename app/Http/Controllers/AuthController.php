...
class AuthController extends Controller
{
    ...
    public function appLogin(Request $request)
    {
        ...
        if (!$user) {
            return response()->json([
                'message' => __('web100now.invalid_login_message'),
                'errors' => __('web100now.invalid_login')
            ], 401);
        }
    ...
    */
    public function login(Request $request)
    {
        ...
        $user->jwt = JWT::encode(['iat' => time(),'exp' => time() + config('web100now.jwt_refresh')], config('web100now.jwt_secret').'-Rfs');
        $user->save();

        return response()->json([
            'access_token' => JWT::encode(['iat' => time(),'exp' => time() + config('web100now.jwt_access')], config('web100now.jwt_secret').'-Acs'),
    ...
    */
    public function refresh(Request $request)
    {
        ...
        if ($user) {
            ...
            return response()->json([
                'message' => __('web100now.invalid_token_message'),
                'errors' => __('web100now.invalid_token')
            ], 401);
        }
    }
    ...
    public function update(Request $request)
    {
        ...
        if ($request->newusername) {
            ...
            if (!Auth::where('username', $newuser)->first()) {
                ...
                return response()->json([
                    'message' => __('web100now.username_conflict_message'),
                    'errors' => __('web100now.username_conflict')
                ], 409);
            }
        }
