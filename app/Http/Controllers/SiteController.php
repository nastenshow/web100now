...
class SiteController extends Controller
{
    ...
    */
    public function create(Request $request)
    {
        ...
        if ($request->php) {
            if (!in_array($request->php, config('web100now.phpvers'))) {
                return response()->json([
                    'message' => __('web100now.bad_request'),
                    'errors' => __('web100now.invalid_php_version')
                ], 400);
            }
            $php = $request->php;
        } else {
            $php = config('web100now.default_php');
        }
        ...
        if ($conflict) {
            return response()->json([
                'message' => __('web100now.site_domain_conflict_message'),
                'errors' => __('web100now.site_domain_conflict')
            ], 409);
        }

        $pdftoken = JWT::encode(['iat' => time(),'exp' => time() + 180], config('web100now.jwt_secret').'-Pdf');
        ...
        $site->username   = config('web100now.users_prefix').hash('crc32', (Str::uuid()->toString())).rand(1, 9);
     ...
     *                  type="string",
     *                  example="andreapollastri/web100now",
     *             ),
     *            @OA\Property(
     *                  property="branch",
    ...
    */
    public function edit(Request $request, string $site_id)
    {
        ...
        if (!$site) {
            return response()->json([
                'message' => __('web100now.site_not_found_message'),
                'errors' => __('web100now.site_not_found')
            ], 404);
        }
            ...
            if ($site->domain != strtolower($request->domain)) {
                ...
                foreach ($sites as $checksite) {
                    if (strtolower($request->domain) == $checksite->domain) {
                        return response()->json([
                            'message' => __('web100now.server_conflict_domain_message'),
                    ...
                    foreach ($checksite->aliases as $alias) {
                        if (strtolower($request->domain) == $alias->domain) {
                            return response()->json([
                                'message' => __('web100now.server_conflict_alias_message'),
                                'errors' => __('web100now.server_conflict')
                            ], 409);
                        }
                    }
    ...
    */
    public function destroy(string $site_id)
    {
        ...
        if ($site->panel) {
            return response()->json([
                'message' => __('web100now.bad_request_default_site_delete'),
    ...
    */
    ...
    {
        try {
            JWT::decode($pdftoken, config('web100now.jwt_secret').'-Pdf', ['HS256']);
        } catch (\Throwable $th) {
            abort(403);
        }
    ...
    */
    public function destroyalias(string $site_id, string $alias_id)
    {
        ...
        if (!$alias) {
            return response()->json([
                'message' => __('web100now.alias_not_found_message'),
                'errors' => __('web100now.alias_not_found')
            ], 404);
        }
