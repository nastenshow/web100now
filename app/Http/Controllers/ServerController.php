...
class ServerController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        if ($validator->fails()) {
            return response()->json([
                'message' => __('web100now.bad_request'),
        ...
        if ($request->ip == $request->server('SERVER_ADDR')) {
            return response()->json([
                'message' => __('web100now.server_conflict_ip_current_message'),
                'errors' => __('web100now.server_conflict')
            ], 409);
        }

        if (Server::where('ip', $request->ip)->first()) {
            return response()->json([
                'message' => __('web100now.server_conflict_ip_duplicate_message'),
                'errors' => __('web100now.server_conflict')
            ], 409);
        }
    ...
    */
    public function destroy(string $server_id)
    {
        ...
        if (!$server) {
            return response()->json([
                'message' => __('web100now.server_not_found_message_default'),
                'errors' => __('web100now.server_not_found')
            ], 404);
        }

        if ($server->default) {
            return response()->json([
                'message' => __('web100now.delete_default_server_message'),
                'errors' => __('web100now.bad_request')
            ], 400);
        }
    ...
    */
    public function show(string $server_id)
    {
        ...
        if (!$server) {
            return response()->json([
                'message' => __('web100now.server_not_found_message'),
                'errors' => __('web100now.server_not_found')
            ], 404);
        }
    ...
    */
    public function panel()
    {
        ...
        if (!$server) {
            return response()->json([
                'message' => __('web100now.server_not_found_native_message'),
                'errors' => __('web100now.server_not_found')
            ], 404);
        }
    ...
    */
    public function panelssl()
    {
        ...
        if ($site) {
            ...
            return response()->json([
                'message' => __('web100now.ssl_request_error_message'),
                'errors' => __('web100now.bad_request')
            ], 400);
        }
    ...
    public function edit(Request $request, string $server_id)
    {
        ...
        if ($request->ip) {
            ...
            if (!$server->default && $request->ip == str_replace("\n", '', file_get_contents('https://checkip.amazonaws.com'))) {
                return response()->json([
                    'message' => __('web100now.edit_server_current_ip_error_message'),
                    'errors' => __('web100now.server_conflict')
                ], 409);
            }
        ...
        if ($request->php) {
            if (!in_array($request->php, config('web100now.phpvers'))) {
                return response()->json([
                    'message' => __('web100now.bad_request'),
    ...
    public function ping(string $server_id)
    {
        ...
        try {
            ...
            if ($remote->status() == 200) {
                ...
                return response()->json([
                    'message' => __('web100now.server_unavailable_message'),
                    'errors' => __('web100now.server_unavailable')
                ], 503);
            }
        } catch (\Throwable $th) {
            return response()->json([
    ...
    */
    public function healthy(string $server_id)
    {
        ...
        try {
            ...
            if (!$ssh->login('web100now', $server->password)) {
                return response()->json([
                    'message' => __('web100now.server_error_ssh_error_message').$server->server_id,
                    'errors' => __('web100now.server_error')
                ], 500);
            }
            $ssh->setTimeout(360);
            ...
            return response()->json([
                'message' => __('web100now.something_error_message'),
                'errors' => __('web100now.error')
            ], 500);
        }
     ...
     *      tags={"Servers"},
     *      description="Reset server root password (for web100now user).",
     *      @OA\Parameter(
     *          name="Authorization",
     *          description="Use Apikey prefix (e.g. Authorization: Apikey XYZ)",
     *          required=true,
     ...
     *              @OA\Property(
     *                  property="password",
     *                  description="New assigned password for web100now root user",
     *                  type="string",
     *                  example="Secret_123"
     *              ),
     *          )
    ...
    */
    ...
    {
        if (!in_array($service, config('web100now.services'))) {
            return response()->json([
                'message' => __('web100now.invalid_service_error_message'),
                'errors' => __('web100now.bad_request')
            ], 400);
        }
