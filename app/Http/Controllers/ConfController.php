...
class ConfController extends Controller
{
    ...
    */
    public function cron(string $server_id)
    {
        ...
        $script = Storage::get('web100now/cron.conf');
        if ($server->cron) {
    ...
    */
    public function panel()
    {
        ...
        $script = Storage::get('web100now/panel.conf');
        $script = Str::replaceArray('???', [
            $site->domain
        ], $script);
    ...
    */
    public function host($site_id)
    {
        ...
        $script = Storage::get('web100now/host.conf');
    ...
    */
    public function php($site_id)
    {
        ...
        $script = Storage::get('web100now/php.conf');
        $script = str_replace('???USER???', $site->username, $script);
        $script = str_replace('???PHP???', $site->php, $script);
        return response($script)->withHeaders(['Content-Type' =>'text/plain']);
    }
    ...
    */
    ...
    {
        $script = Storage::get('web100now/nginx.conf');
        return response($script)->withHeaders(['Content-Type' =>'text/plain']);
    }
    ...
    */
    ...
    {
        $script = Storage::get('web100now/supervisor.conf');
        return response($script)->withHeaders(['Content-Type' =>'text/plain']);
    }
}
