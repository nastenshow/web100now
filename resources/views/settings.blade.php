    ...
    <div class="col-xl-6">
        <div class="card mb-4">
            <div class="card-header">
                ...
                {{ __('web100now.change_username') }}
            </div>
            <div class="card-body">
                <p>{{ __('web100now.current_username') }}: <b><span id="currentuser"></span></b></p>
                <div class="input-group">
                    <input class="form-control" type="text" placeholder="{{ __('web100now.change_username_placeholder') }}" id="newuser" autocomplete="off" />
    ...
    <div class="col-xl-6">
        <div class="card mb-4">
            <div class="card-header">
                ...
                {{ __('web100now.change_password') }}
            </div>
            <div class="card-body">
                <p>{{ __('web100now.update_password') }}</p>
                <div class="input-group">
                    <input class="form-control" type="text" placeholder="{{ __('web100now.change_password_placeholder') }}" id="newpass" autocomplete="off" />
                    <div class="input-group-append">
    ...
    <div class="col-xl-6">
        <div class="card mb-4">
            <div class="card-header">
                ...
                {{ __('web100now.panel_url') }}
            </div>
            <div class="card-body">
                <p>{{ __('web100now.panel_url_text') }}</p>
                <div class="input-group">
                    <input class="form-control" type="text" placeholder="panel.domain.ltd" id="panelurl" autocomplete="off" />
                    <div class="input-group-append">
                        <button class="btn btn-warning" type="button" id="panelurlssl" data-toggle="tooltip" data-placement="top" title="{{ __('web100now.panel_url_force_ssl') }}"><i class="fas fa-lock"></i></button>
                    </div>
                    <div class="input-group-append">
    ...
    <div class="col-xl-6">
        <div class="card mb-4">
            <div class="card-header">
                ...
                {{ __('web100now.panel_api') }}
            </div>
            <div class="card-body">
                <p>{{ __('web100now.panel_api_endpoint') }}:  <b>{{ URL::to('/api/') }}</b></p>
                <div class="text-center">
                    <button class="btn btn-primary mr-3" type="button" id="newapikey">{{ __('web100now.renew_api_key') }}</button>
                    <a href="/api/docs" target="_blank">
                        <button class="btn btn-warning" type="button">{{ __('web100now.documentation') }}</button>
                    </a>
                </div>
                <div class="space"></div>
            </div>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="authorizeModalLabel">{{ __('web100now.action_authorization_modal_title') }}</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <p>{{ __('web100now.action_authorization_modal_text') }}:</p>
                <div class="input-group">
                    <input class="form-control" type="password" id="currentpass" />
                </div>
                <div class="space"></div>
                <div class="text-center">
                    <button class="btn btn-primary" type="button" id="submit">{{ __('web100now.submit') }} <i class="fas fa-circle-notch fa-spin d-none" id="loading"></i></button>
                </div>
                <div class="space"></div>
            </div>
        </div>
        ...
        if(!oldpassval || oldpassval.length < 8) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if(patchcall == 'changeuser') {
                        success('{{ __('web100now.username_updated_success') }}');
                        $('#currentuser').html(localStorage.username);
                        $('#username').html(localStorage.username);
                    }
                    if(patchcall == 'changepass') {
                        success('{{ __('web100now.password_updated_success') }}');
                    }
                    if(patchcall == 'newapikey') {
                        success('{{ __('web100now.new_api_key_success') }}:<br><b>'+data.apikey+'</b>');
                    }
                ...
                error: function(error) {
                    if(error.status == 401) {
                        ...
                        $('#newpass').val('');
                        $('#currentpass').val('');
                        fail('{{ __('web100now.unknown_error') }}');
                        $('#authorizeModal').modal("hide");
                    }
                }
            });
